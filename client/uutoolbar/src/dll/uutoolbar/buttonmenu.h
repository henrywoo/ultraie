...
#if !defined(AFX_BUTTONMENU_H_INCLUDED_)
#define AFX_BUTTONMENU_H_INCLUDED_
...
public:
   ...
   void SetMenu(HMENU hMenu)
   {
      ...
      m_menu.Attach(hMenu);
      m_bIsMenuOwner = false;
      //::MessageBox(NULL,_T("menu"),_T("menu"),MB_ICONINFORMATION);
   }
   ...
   void _Init()
   {
      ...
      wcscpy_s(lf.lfFaceName, _T("Webdings"));
   ...
   {
      ::MessageBox(NULL,_T("OnClick call MenuButton"),_T("menu"),MB_ICONINFORMATION);// simonwoo 
   ...
   LRESULT OnMenuDrawItem(UINT , WPARAM , LPARAM lParam, BOOL& )
   {
      ...
      UINT iIndex = (UINT)lpDIS->itemData;//
   ...
   DWORD OnPrePaint(int, LPNMCUSTOMDRAW /*lpNMCD*/)
   {
   ...
   //画button表面
   //重载 CCustomDraw 的方法
   DWORD OnPostPaint(int, LPNMCUSTOMDRAW lpNMCD)
   {
      CDCHandle dc = lpNMCD->hdc;
      RECT rc = lpNMCD->rc;
      RECT rcLine = { rc.right - 22, rc.top + 3, rc.right - 20, rc.bottom - 3 };
      ...
      dc.SetBkMode(TRANSPARENT);
      dc.SetTextColor(::GetSysColor(bDisabled ? COLOR_GRAYTEXT : COLOR_BTNTEXT));
      dc.DrawText(ARROW_CHAR, 1, &rcArrow, DT_SINGLELINE | DT_LEFT | DT_VCENTER);//向下的箭头符号

      //simonwoo added code here...
      RECT simonrect = { rc.left+13, rc.top+3, rc.left+13+16, rc.bottom-3};
      dc.SetBkMode(TRANSPARENT);
      dc.SetTextColor(::GetSysColor(bDisabled ? COLOR_GRAYTEXT : COLOR_BTNTEXT));
      dc.DrawText(ARROW_CHAR, 1, &simonrect, DT_SINGLELINE | DT_LEFT | DT_VCENTER);//向下的箭头符号
...
#endif // !defined(AFX_BUTTONMENU_H_INCLUDED_)
