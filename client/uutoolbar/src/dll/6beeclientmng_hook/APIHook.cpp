...
CAPIHook::~CAPIHook() {
  ...
  if (p == this) {     // Removing the head node
    ...
    for (; !bFound && (p->m_pNext != NULL); p = p->m_pNext) {
      if (p->m_pNext == this) { // Make the node that points to us point to our next node
...
LONG WINAPI InvalidReadExceptionFilter(PEXCEPTION_POINTERS pep) {
  // handle all unexpected exceptions because we simply don't patch any module in that case
...
void CAPIHook::ReplaceEATEntryInOneMod(HMODULE hmod, PCSTR pszFunctionName, PROC pfnNew) {
  ...
  __try {
    ...
    // Nothing to do in here, thread continues to run normally with NULL for pExportDir 
  }
  if (pExportDir == NULL)
...
// This make my computer extremely slow!
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
// Hook LoadLibrary functions and GetProcAddress so that hooked functions are handled correctly if these functions are called.
//CAPIHook CAPIHook::sm_LoadLibraryA  ("Kernel32.dll", "LoadLibraryA",   (PROC) CAPIHook::MyLoadLibraryA);
//CAPIHook CAPIHook::sm_LoadLibraryW  ("Kernel32.dll", "LoadLibraryW",   (PROC) CAPIHook::MyLoadLibraryW);
//CAPIHook CAPIHook::sm_LoadLibraryExA("Kernel32.dll", "LoadLibraryExA", (PROC) CAPIHook::MyLoadLibraryExA);
//CAPIHook CAPIHook::sm_LoadLibraryExW("Kernel32.dll", "LoadLibraryExW", (PROC) CAPIHook::MyLoadLibraryExW);
//CAPIHook CAPIHook::sm_GetProcAddress("Kernel32.dll", "GetProcAddress", (PROC) CAPIHook::MyGetProcAddress);
...
///////////////////////////////////////////////////////////////////////////////
HMODULE WINAPI CAPIHook::MyLoadLibraryA(PCSTR pszModulePath) {
  HMODULE hmod = ::LoadLibraryA(pszModulePath);
  FixupNewlyLoadedModule(hmod, 0);
  return(hmod);
}
///////////////////////////////////////////////////////////////////////////////
HMODULE WINAPI CAPIHook::MyLoadLibraryW(PCWSTR pszModulePath) {
  HMODULE hmod = ::LoadLibraryW(pszModulePath);
  FixupNewlyLoadedModule(hmod, 0);
  return(hmod);
}
///////////////////////////////////////////////////////////////////////////////
HMODULE WINAPI CAPIHook::MyLoadLibraryExA(PCSTR pszModulePath, HANDLE hFile, DWORD dwFlags) {
  HMODULE hmod = ::LoadLibraryExA(pszModulePath, hFile, dwFlags);
  FixupNewlyLoadedModule(hmod, dwFlags);
  return(hmod);
}
///////////////////////////////////////////////////////////////////////////////
HMODULE WINAPI CAPIHook::MyLoadLibraryExW(PCWSTR pszModulePath, HANDLE hFile, DWORD dwFlags) {
  HMODULE hmod = ::LoadLibraryExW(pszModulePath, hFile, dwFlags);
  FixupNewlyLoadedModule(hmod, dwFlags);
  return(hmod);
}
///////////////////////////////////////////////////////////////////////////////
FARPROC WINAPI CAPIHook::MyGetProcAddress(HMODULE hmod, PCSTR pszProcName) {
