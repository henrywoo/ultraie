  ...
  void DrawItem (LPDRAWITEMSTRUCT lpDrawItemStruct)
  {
    ...
    else
    {
      ...
      */			
      ...
      USES_CONVERSION;
      char a[100];
      _itoa_s(pRect->bottom-pRect->top,a,10);
      ::MessageBox(NULL,A2CW(a),_T("HELLO"),0);
      */
    ...
    if (pmd->fType & MFT_SEPARATOR)
    {
      ...
      */
      ...
      rc.left = m_szBitmap.cx + IMGPADDING + TEXTPADDING;//写字
      ...
      if (m_ImageList != NULL && pmd->iImage >= 0)
      {
        ...
        */
        ...
        if (bDisabled || (bSelected && !bChecked))
        {
          ...
          COLORREF c1 = HLS_TRANSFORM(::GetSysColor(COLOR_HIGHLIGHT), +50, -66);
          COLORREF c2 = HLS_TRANSFORM(::GetSysColor(COLOR_3DFACE), -27, 0);
          brush.CreateSolidBrush(bOver ? c1 : c2);
      ...
      else if (bChecked)
      {
        ...
        BOOL bRet = pDC->DrawState(CPoint(rc.left,rc.top + 3), CSize(rc.Size()),
  ...
  LRESULT InitMenuPopupHandler(UINT uMsg,WPARAM wParam,LPARAM lParam,BOOL& bHandled)
  {
    ...
    for (int i = 0; i < menuPopup.GetMenuItemCount(); i++)
    {
      ...
      if (!(mii.fType & MFT_OWNERDRAW))   // not already an ownerdraw item
      {
        ...
        coolMenuItemData * pMI = new coolMenuItemData;
        ATLASSERT(pMI != NULL);
