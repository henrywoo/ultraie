...
BOOL CBlockingSocketException::GetErrorMessage(wchar_t* lpstrError, UINT nMaxError,
...
std::wstring CBlockingSocketException::GetErrorMessage(PUINT /*pnHelpContext = NULL*/){
...
int CBlockingSocket::Write(const char* pch, int nSize, int nSecs, bool terminatethisthread) const
{
	int         nBytesSent        = 0;
	int         nBytesThisTime;
	const char* pch1              = pch;
	...
	do{
		if (terminatethisthread){
			char *q = "QUIT";
			Send(q, strlen(q)+1, nSecs);
			//this->Close();
			return -1;//return -1 means the thread will be terminated
		}
...
CSockAddr CBlockingSocket::GetHostByName(const char* pchName, unsigned short ushPort /* = 0 */)
{
...
*/
IBlockingSocket* _6bees_socket::CreateDefaultBlockingSocketInstance(){
	return new CBlockingSocket();
}
