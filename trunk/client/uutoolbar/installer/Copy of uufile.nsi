...
Name          "UUFile文件上传工具"
Caption       "UUFile文件上传工具安装"
Icon          "install.ico"
UninstallIcon "uninstall.ico"
OutFile       "uufileuploader_installer.exe"
...
InstallDirRegKey HKLM "Software\6BeeNetworks" "Location"
...
Function DetectVersion
  ClearErrors
  ReadRegStr $0 HKLM "SOFTWARE\6BeeNetworks" "Version"
...
instdirnotexisting:
  ...
  ;File ${DEPLOY_ROOT}\uufile_clientmanger.exe
...
Section "Start Menu Shortcuts"
  ;Add to start menu
  CreateDirectory "$SMPROGRAMS\uufile"
  ;CreateShortCut "$SMPROGRAMS\uufile\登陆UUFile.lnk" "$INSTDIR\uufile_clientmanger.exe" "" "$INSTDIR\uufile_clientmanger.exe" 0
  CreateShortCut "$SMPROGRAMS\uufile\悠悠上传.lnk" "$INSTDIR\uufile_uploader.exe" "" "$INSTDIR\uufile_uploader.exe" 0
  ;CreateShortCut "$SMPROGRAMS\uufile\卸载.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
  
  ;CreateShortCut "$SMPROGRAMS\uufile\UUFILE在线文件存储空间.lnk" "${PRODUCT_WEB_SITE}/upload.php" "" "$INSTDIR\uufile_clientmanger.exe" 0
  
  ;Add to desktop
  ;CreateShortCut "$DESKTOP\登陆UUFile.lnk" "$INSTDIR\uufile_clientmanger.exe" "" "$INSTDIR\uufile_clientmanger.exe" 0
  CreateShortCut "$DESKTOP\悠悠上传.lnk" "$INSTDIR\uufile_uploader.exe" "" "$INSTDIR\uufile_uploader.exe" 0
  
  ;Favorite
  ;CreateShortCut "$FAVORITES\uufile\悠悠花园 开放注册.lnk" "$INSTDIR\悠悠花园开放注册.url" "" "$PROGRAMFILES\Internet Explorer\iexplore.exe" 0
  
  ;CreateShortCut "$QUICKLAUNCH\登陆UUFile.lnk" "$INSTDIR\uufile_clientmanger.exe" "" "$INSTDIR\uufile_clientmanger.exe" 0
  CreateShortCut "$QUICKLAUNCH\悠悠上传.lnk" "$INSTDIR\uufile_uploader.exe" "" "$INSTDIR\uufile_uploader.exe" 0
...
Section "Uninstall"
  ...
  ; Give an option
  Delete $INSTDIR\*
  Delete $SMPROGRAMS\uufile\*
  Delete $SMPROGRAMS\uufile
  Delete "$DESKTOP\悠悠上传.lnk"
  Delete "$QUICKLAUNCH\悠悠上传.lnk"
  
  ; Remove shortcuts, if any
  Delete "$SMPROGRAMS\uufile\*.*"
  ; Remove directories used
  RMDir "$SMPROGRAMS\uufile"
  RMDir /r /REBOOTOK "$INSTDIR"
SectionEnd

Function WriteuufileReg
WriteRegStr HKLM SOFTWARE\6BeeNetworks "Location" "$INSTDIR\" ; must have a back slash
WriteRegStr HKLM SOFTWARE\6BeeNetworks "Version" "$PVERSION"
...
DeleteRegValue HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings\5.0\User Agent\Post Platform" "uufile(6BEENETWORKS 2009)"
