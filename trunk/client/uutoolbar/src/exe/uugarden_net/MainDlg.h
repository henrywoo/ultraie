...
public:
  ...
  LRESULT OnInitDialog(UINT,WPARAM,LPARAM,BOOL&){
    ...
    m_AccountTxt.Attach(GetDlgItem(IDC_ACCOUNTTXT));
    m_PassTxt.Attach(GetDlgItem(IDC_PASSTXT));

    m_Note.Attach(GetDlgItem(IDC_NOTE));
    m_CUserName.Attach(GetDlgItem(IDC_USERNAME_));
    m_CUserName.SetFocus();
    m_CUserName.SetWindowText(L"");
  ...
  void PostLogin(std::string& username,std::string& passwordtostore){
    ...
    m_PassTxt.ShowWindow(SW_HIDE);
    m_AccountTxt.ShowWindow(SW_HIDE);
