 ...
 /*
  ...
  void OnMouseMove( UINT nFlags, CPoint point){
    ...
    if(m_bButtonDown){
      ...
      if(!HitTest( point, nItem, nSubItem)){
        ...
        CString strURLText = pT->GetItemText(nItem,colLink/*URL*/);
  ...
  void DrawItem( CDCHandle dcPaint, int nItem, CRect& rcItem){
    ...
    if(bSelectedItem){
      if(!m_thmHeader.IsThemeNull()){
        // draw select border
        _6beed_util::DrawRoundRect( dcPaint.m_hDC, rcItem, bControlFocus ? m_rgbSelectOuter : m_rgbNoFocusOuter, bControlFocus ? m_rgbSelectInner : m_rgbNoFocusInner,m_rgbBackground);

        CRect rcSelect( rcItem);
        rcSelect.DeflateRect( 2, 2);
        // fill selected area
        _6beed_util::DrawGradient( dcPaint.m_hDC, &rcSelect, bControlFocus ? m_rgbSelectTop : m_rgbNoFocusTop, bControlFocus ? m_rgbSelectBottom : m_rgbNoFocusBottom);
      }else{
    ...
    for(int nSubItem = 0, nColumnCount = GetColumnCount();
      nSubItem < nColumnCount;
      rcSubItem.left = rcSubItem.right + 1, nSubItem++)
    {
      CListColumn listColumn;
      ...
      if(bFocusSubItem){//if select some item...
        if(!m_thmHeader.IsThemeNull()){//always use the beautiful style!!! -- simonwoo
          ...
          if(bControlFocus){
            // draw select border
            _6beed_util::DrawRoundRect( dcPaint.m_hDC, rcSubItem, bControlFocus ? m_rgbSelectOuter : m_rgbNoFocusOuter, bControlFocus ? m_rgbSelectInner : m_rgbNoFocusInner, m_rgbBackground);
            CRect rcSelect( rcSubItem);
            rcSelect.DeflateRect( 2, 2);
            // fill selected area
            _6beed_util::DrawGradient( dcPaint.m_hDC, &rcSelect, m_rgbFocusTop, m_rgbFocusBottom);
          }
        }else{
        ...
        case ITEM_FORMAT_PROGRESS:
          {
            if (nProgress>0){
              ...
              _6beed_util::DrawRoundRect( dcPaint.m_hDC, rcProgress, m_rgbHeaderShadow, m_rgbHeaderBackground,m_rgbBackground);
              ...
              _6beed_util::DrawGradient( dcPaint.m_hDC, &rcProgress, m_rgbProgressTop, m_rgbProgressBottom);
