...
#ifdef _DEBUG
#define INSERT_TIME() \
if( ftpFileStatus.m_mtime>0 ) ftpFileStatus.m_strMTime = CCnv::ConvertToTString(asctime(gmtime(&ftpFileStatus.m_mtime)));
#else
#define INSERT_TIME()
#endif
#endif
...
{
   std::wstring strLong(pszLong, iLength);
   
   wchar_t* pszEndPtr = NULL;
 lResult = wcstoul(strLong.c_str(), &pszEndPtr, 10);
...
bool CFTPListParse::ParseUNIXStyleListing(CFTPFileStatus& ftpFileStatus, LPCTSTR pszLine, int iLength)
{
 ...
 return false;
   ...
   if( pszLine[0] == _T('l') )
   {
 std::wstring::size_type pos = ftpFileStatus.m_strName.find(_T(" -> "));
...
{
   return pszLine && wcschr(pszLine, _T(';')) != NULL;
}
...
bool CFTPListParse::ParseMultiNetListing(CFTPFileStatus& ftpFileStatus, LPCTSTR pszLine, int iLength)
{
 ...
 if( i > 4 && wcsncmp(pszLine+i-4, _T(".DIR"), 4)==0 )
 {
...
bool CFTPListParse::IsMSDOSListing(LPCTSTR pszLine){
   return pszLine && iswdigit(pszLine[0]);
}
...
bool CFTPListParse::Parse(CFTPFileStatus& ftpFileStatus, const std::wstring& strLineToParse)
{
   ftpFileStatus.Reset();
