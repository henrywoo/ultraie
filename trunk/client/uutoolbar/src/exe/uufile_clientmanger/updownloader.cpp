...
void PopDlgtoSelectFiles(uploaderdlg& dlgMain,ATL::CString& strFolderPath,ATL::CString& strFileName,int& mssize);

void termin_func(){
  _6bees_window::NotifyClientMng(CPYDATA_DATATRANSFER_TERM,_6bees_const::kClientMng_WndTitle,NULL);
...
int CallUploader(const wchar_t* cmdline){
  ...
  ParseCmd(stringmaker(CP_ACP)<<cmdline,mode,&vec_files);
...
bool ParseCmd(string cmdline,
                  ...
                  vector<wstring>* r)
{
  mode = UPLOAD_MODE;//default to upload mode
  if(r==NULL)
    return false;
