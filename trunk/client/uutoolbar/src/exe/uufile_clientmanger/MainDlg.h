...
const wchar_t* const tip_offline = L"您还未登陆到UUFile!";
const wchar_t* const tip_loggedin = L"您已经登陆到UUFile!";
const wchar_t* const tip_ntwkfail= L"登陆失败,请确保您的网络连接,用户名和密码正确.";
const wchar_t* const tip_logining= L"登录中,请稍等...";
const wchar_t* const tip_welcome= L"欢迎%s:\n现在您已经登陆到UUFile!";
const wchar_t* const tip_plslogin= L"请用您注册时填写的电子邮箱和密码登陆!";
const wchar_t* const tip_huany= L"欢迎 ";
const wchar_t* const tip_logout= L"登出UUFILE";
const wchar_t* const tip_logging= L"账号%s正在登陆请稍等...";
const wchar_t* const tip_plsreg= L"请登陆,如果还没有账号请去UUFile注册!";
const wchar_t* const tip_login= L"登陆UUFile";
const wchar_t* const tip_uploadfinish= L"文件上传成功";
...
public:
  ...
  LRESULT OnInitDialog(UINT,WPARAM,LPARAM,BOOL&){
    ...
    HBITMAP hbmp2=(HBITMAP)c6beeres::R().GetImg(BUTTON_CLNMNG_CH,IMAGE_BITMAP,45*4,22);
    ...
    hi=(HICON)c6beeres::R().GetImg(
      u_getLoggedin()?IDI_6BEELOGO:IDI_6BEEDISABLED,
      IMAGE_ICON,::GetSystemMetrics(SM_CXSMICON),::GetSystemMetrics(SM_CYSMICON));
    ATL::CString bonjour(tip_offline);
    if(u_getLoggedin())
      bonjour.Format(tip_welcome,uu);
    ...
    m_CLogIn.SetToolTipText(tip_login);
    ...
    m_CRegister.SetToolTipText(tip_plsreg);
    ...
    if (u_getLoggedin()||u_getAutologin()){
      ...
      ModifyStyleEx(WS_EX_APPWINDOW,WS_EX_TOOLWINDOW);//从task bar/atl+tab中去掉.
    }
#endif
    CMessageLoop* pLoop = _Module.GetMessageLoop();
    ...
    if(registereduser){
      ...
      if(0){
        wchar_t notice[100]={};
        swprintf_s(notice,100,tip_logging,uu);
      ...
      m_Note.SetWindowText(tip_plsreg);
    }
    //UIAddChildWindowContainer(m_hWnd);
    ...
    _6beed_util::CheckVersion(pszLoader);
  ...
  LRESULT OnCopyData(UINT uMsg,WPARAM wParam,LPARAM lParam,BOOL& bHandled){
    ...
    switch (a){
      ...
      case CPYDATA_FILEUPLOAD_OK:
        ...
        m.resize(255);
        PopMsg(m.c_str(),tip_uploadfinish);
        }break;
  ...
  void DynamicChangeMenuItem(CMenuItemInfo& mii, coolMenuItemData * pMI, TCHAR* szString,bool& ignore){
    if (u_getLoggedin()){
      if (mii.wID==ID_A_LOGINOUT){
        const wchar_t* const a=tip_logout;
...
#if 0
    ATL::CString bonjour;
    bonjour.Format(tip_welcome,unamew_.c_str());
    SetTooltipText(bonjour);
    unamew_ = tip_huany + unamew_;
    PopMsg(tip_loggedin,unamew_.c_str());
    m_Note.SetWindowText(tip_plslogin);
...
UINT WINAPI ThreadLogin(LPVOID lp){
  try{
    ...
    if(_user!=0){
      std::string sid;
      if(_user->LoginUUFile(uuname,ppword,sid)==::UUTOOLBAR::LOGINOK){
